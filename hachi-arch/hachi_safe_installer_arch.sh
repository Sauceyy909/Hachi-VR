...
# HACHI SAFE INSTALLER - Arch/Garuda Linux Compatible
...
echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  HACHI SAFE INSTALLER (Arch/Garuda)   ║${NC}"
...
echo "  ✗ Modify system packages (except tk)"
...
if ! command -v python3 &> /dev/null; then
    ...
    echo "Please install Python3 first: sudo pacman -S python"
    exit 1
fi
PYTHON_VERSION=$(python3 --version | awk '{print $2}')
echo -e "${GREEN}✓ Python3 found (version $PYTHON_VERSION)${NC}"

# Install tk (REQUIRED for GUI)
echo -e "\n${YELLOW}Installing tk (required for GUI)...${NC}"
if python3 -c "import tkinter" 2>/dev/null; then
    echo -e "${GREEN}✓ tk already installed${NC}"
else
    echo -e "${YELLOW}Installing tk via pacman...${NC}"
    echo "This is the ONLY system package we install (safe, GUI-only)"
    
    if command -v pacman &> /dev/null; then
        sudo pacman -Sy --needed --noconfirm tk 2>&1 | tee -a "$LOG_FILE"
        
        if python3 -c "import tkinter" 2>/dev/null; then
            echo -e "${GREEN}✓ tk installed successfully${NC}"
        else
            echo -e "${RED}✗ Failed to install tk${NC}"
            echo "Please install manually: sudo pacman -S tk"
            exit 1
        fi
    else
        echo -e "${RED}✗ pacman not found (are you on Arch/Garuda?)${NC}"
        echo "Please install tk manually"
        exit 1
    fi
fi
...
if [ -n "$VENV_DIR" ]; then
    ...
    if pip3 install --user pyusb opencv-python numpy 2>&1 | tee -a "$LOG_FILE"; then
        ...
        # This flag might be needed on newer systems
...
if ! python3 -c "import tkinter" 2>/dev/null; then
    echo "ERROR: Python tkinter (tk) not found!"
...
echo "  ✓ tk (system package for GUI)"
if [ -n "$VENV_DIR" ]; then
